...
**Query** : SELECT f.rfam_acc AS family_accession, f.description AS family_name, MAX(r.length) AS max_sequence_length FROM    family AS f JOIN full_region AS fr ON f.rfam_acc = fr.rfam_acc
JOIN rfamseq AS r ON fr.rfamseq_acc = r.rfamseq_acc GROUP BY
f.rfam_acc, f.description HAVING max_sequence_length > 1000000
ORDER BY max_sequence_length DESC LIMIT 120, 15;
